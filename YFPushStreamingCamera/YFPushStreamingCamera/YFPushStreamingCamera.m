...
@interface YFSampleBufferCallback: NSObject <AVCaptureVideoDataOutputSampleBufferDelegate>
{
    YFPushStreamingCamera *m_source;
}

- (void)setSource:(YFPushStreamingCamera *)source;
@end

@implementation YFSampleBufferCallback
...
{
    if (m_source) {
        [m_source bufferCaptured:CMSampleBufferGetImageBuffer(sampleBuffer)];
    }
}
...
@end
// /////////////////////////////////////////////////////
@interface YFPushStreamingCamera ()
{
    ...
    YFPushStreamingCameraState  _cameraState;
    BOOL                        _cameraTorch;
}

/**
 * @brief   设置旋转锁定的状态
 * @param   INPUT   orientationLocked   设置是否锁定旋转
 * @return  无返回
 */
 ...
 * @param   INPUT   x   对焦位置的横坐标
 * @param   INPUT   y   对焦位置的纵坐标
...
/**
 * @brief   设置自动对焦点
 * @param   INPUT   wantsContinuous   true为自动对焦
 * @return  设置成功返回true，失败返回false
 */
...
/**
 * @brief   设置曝光点
 * @param   INPUT   x   曝光点位置的横坐标
 * @param   INPUT   y   曝光点位置的纵坐标
 * @return  设置成功返回true，失败返回false
 */
...
/**
 * @brief   设置自动曝光点
 * @param   INPUT   wantsContinuous   true为自动曝光
 * @return  设置成功返回true，失败返回false
 */
- (bool)setContinuousExposure:(bool)wantsContinuous;
@end

@implementation YFPushStreamingCamera
- (instancetype)init
{
    self = [super init];
    if (self) {
...
- (void)dealloc
{
    ...
    if(m_callbackSession) {
        ...
        [((YFSampleBufferCallback*)m_callbackSession) release];
    }
    if(m_previewLayer) {
...
- (YFPushStreamingCameraState)cameraState
{
    return _cameraState;
}
- (void)setCameraState:(YFPushStreamingCameraState)cameraState
{
...
{    
    ...
    __block YFPushStreamingCamera* bThis = self;
        ...
        @autoreleasepool {
            if(granted) {
                ...
                if(!bThis->m_callbackSession) {
                    bThis->m_callbackSession = [[YFSampleBufferCallback alloc] init];
                    [((YFSampleBufferCallback*)bThis->m_callbackSession) setSource:self];
                }
                dispatch_queue_t camQueue = dispatch_queue_create("com.YFPushStreaming.camera", 0);
                
                [output setSampleBufferDelegate:((YFSampleBufferCallback*)bThis->m_callbackSession) queue:camQueue];
...
{
    [self setOutput:(void*)pixelBufferRef];
}

- (void)setOutput:(CVPixelBufferRef)pixelBufferRef
{
    [self.delegate pushStreamingCameraSourceOutput:pixelBufferRef];
}
